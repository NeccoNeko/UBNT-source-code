...
enum inject_status {
	INJECT_STATUS_DIS = 0,
	INJECT_STATUS_EN_UNCORR = 1,
	INJECT_STATUS_EN_CORR = 2,
};
...
struct al_mc_drvdata {
	...
	enum inject_status	inject_status;
};
...
static irqreturn_t al_mc_uncorr_err_handler(int irq, void *dev_id)
{
	...
	schedule_delayed_work(&drvdata->handle_uncorr_err_work, msecs_to_jiffies(1));
...
static void al_mc_corr_err_work(struct work_struct *work)
{
	...
	/* get the ecc status */
	al_ddr_ecc_status_get(&drvdata->ddr_cfg,
			&ecc_corr_status, NULL);

	al_ddr_ecc_corr_count_clear(&drvdata->ddr_cfg);

	al_ddr_address_translate_dram2sys(&drvdata->ddr_cfg, &err_addr,
			ecc_corr_status.rank, ecc_corr_status.bank, ecc_corr_status.bg,
	...
	al_ddr_ecc_corr_int_clear(&drvdata->ddr_cfg);
}
...
static void al_mc_uncorr_err_work(struct work_struct *work)
{
	...
	al_ddr_ecc_uncorr_count_clear(&drvdata->ddr_cfg);

	al_ddr_address_translate_dram2sys(&drvdata->ddr_cfg, &err_addr,
			ecc_uncorr_status.rank, ecc_uncorr_status.bank, ecc_uncorr_status.bg,
	...
	al_ddr_ecc_uncorr_int_clear(&drvdata->ddr_cfg);
}
	...
	const char *data, size_t count)
{
	...
	rc = al_ddr_address_translate_sys2dram(&drvdata->ddr_cfg, addr,
			&drvdata->inject.rank, &drvdata->inject.bank, &drvdata->inject.bg,
			&drvdata->inject.col, &drvdata->inject.row);
	if (rc < 0)
		return -EIO;

	if (drvdata->inject_status != INJECT_STATUS_DIS) {
		rc = al_ddr_ecc_data_poison_enable(
				...
				drvdata->inject.col,
				drvdata->inject.row,
				(drvdata->inject_status == INJECT_STATUS_EN_CORR) ?
				AL_TRUE : AL_FALSE);
		if (rc < 0)
			return -EIO;
	}
	...
	char *data)
{
	...
	al_ddr_address_translate_dram2sys(&drvdata->ddr_cfg, &addr,
			drvdata->inject.rank, drvdata->inject.bank, drvdata->inject.bg,
			drvdata->inject.col, drvdata->inject.row);
	...
	const char *data, size_t count)
{
	...
	if (data_val != INJECT_STATUS_DIS)
		rc = al_ddr_ecc_data_poison_enable(
				...
				drvdata->inject.col,
				drvdata->inject.row,
				(data_val == INJECT_STATUS_EN_CORR) ?
				AL_TRUE : AL_FALSE);
	else
		rc = al_ddr_ecc_data_poison_disable(&drvdata->ddr_cfg);

	drvdata->inject_status = data_val;
	if (rc < 0)
		return rc;
	...
	char *data)
{
	...
	return sprintf(data, "%d (%s)\n", drvdata->inject_status,
		(drvdata->inject_status == INJECT_STATUS_DIS) ? "disabled" :
		(drvdata->inject_status == INJECT_STATUS_EN_CORR) ? "enabled - correctable" :
		"enabled - uncorrectable");
}
...
#define al_inject_store_impl(_field)					\
	do {								\
		...
		drvdata->inject_status = INJECT_STATUS_DIS;		\
		rc = al_ddr_ecc_data_poison_disable(&drvdata->ddr_cfg);	\
		if (rc < 0)						\
			return -EIO;					\
	} while (0)
...
static ssize_t al_inject_bg_store(
	struct device *dev,
	struct device_attribute *mattr,
	const char *data, size_t count)
{
	al_inject_store_impl(bg);
...
static ssize_t al_inject_bg_show(
	struct device *dev,
	struct device_attribute *mattr,
	char *data)
{
	al_inject_show_impl(bg);
}
	...
	char *data)
{
	return sprintf(data,
		...
		"- inject_col/rank/row/bank/bg\n"
		"\tset/show ddr col/rank/row/bank/bg value for UC error injection\n"
		"- inject_enable\n"
		"\tcorrectable/uncorrectable/disable error injection "
		"(by setting to %u/%u/%u), or print\n"
		"\tcurrent state\n"
		"\t(*)when changing an address configuration, you need to\n"
		"\t   re-enable the interface i.o. to apply your changes\n",
		INJECT_STATUS_EN_CORR,
		INJECT_STATUS_EN_UNCORR,
		INJECT_STATUS_DIS);
}
...
static DEVICE_ATTR(inject_bg, DEVICE_ATTR_FLAGS,
		al_inject_bg_show, al_inject_bg_store);
...
static void al_delete_sysfs_devices(struct mem_ctl_info *mci)
{
	...
	device_remove_file(&mci->dev, &dev_attr_inject_bg);
...
static int al_create_sysfs_devices(struct mem_ctl_info *mci)
{
	...
	if (rc < 0)
		goto err;
	rc = device_create_file(&mci->dev, &dev_attr_inject_bg);
	if (rc < 0)
		goto err;
...
static int al_mc_probe(struct platform_device *pdev)
{
	...
	fabric_regs_base = al_fabric_reg_base_get();
	if (!fabric_regs_base) {
		dev_err(&pdev->dev, "fabric regs not available\n");
		res = -ENOMEM;
		goto err;
	}
	...
	res = al_ddr_cfg_init(
		...
		&ddr_cfg);
	if (res) {
		dev_err(&pdev->dev, "al_ddr_cfg_init failed!\n");
		goto err;
	}
	...
	active_ranks = al_ddr_active_ranks_get(&ddr_cfg);
	if (!active_ranks) {
	...
	rank_addr_bits = al_ddr_bits_per_rank_get(&drvdata->ddr_cfg);
	/*
	 * Set dimm attributes
	 *   - Use DDR3 type memory
	 *   - Single-bit Error Correction, Double-bit Error Detection (SECDED)
	...
	for (i = 0 ; i < active_ranks ; i++) {
		...
		dimm->grain = (al_ddr_data_width_get(&drvdata->ddr_cfg) ==
