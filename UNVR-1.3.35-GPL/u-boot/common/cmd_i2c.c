...
static int do_i2c_write(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (argc == 6 && !strcmp(argv[5], "-s")) {
		...
		if (ret)
			return i2c_report_err(ret, I2C_ERR_WRITE);
	} else if (argc == 7 && !strcmp(argv[5], "-p")) {
		unsigned int pg_size = simple_strtoul(argv[6], NULL, 10);

		while (length) {
			unsigned int next_pg_addr =
				((devaddr + pg_size) / pg_size) * pg_size;
			unsigned int tx_size = next_pg_addr - devaddr;

			if (length < tx_size)
				tx_size = length;

			ret = i2c_write(chip, devaddr, alen, memaddr, tx_size);
			if (ret)
				return i2c_report_err(ret, I2C_ERR_WRITE);
			udelay(11000);
...
/**
 * do_i2c_sda_hold_time() - Handle the "i2c sda_hold_time" command-line command
 * @cmdtp:	Command data struct pointer
 * @flag:	Command flag
 * @argc:	Command-line argument count
 * @argv:	Array of command-line arguments
 ...
 * on error.
 */
static int do_i2c_sda_hold_time(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	int sda_hold_time, ret = 0;

	if (argc == 1) {
		sda_hold_time = i2c_get_sda_hold_time();
		/* querying current sda_hold_time */
		printf("Current SDA hold time=%d\n", sda_hold_time);
	} else {
		sda_hold_time = simple_strtoul(argv[1], NULL, 10);
		printf("Setting SDA hold time to %d ns\n", sda_hold_time);
		ret = i2c_set_sda_hold_time(sda_hold_time);
		if (ret)
			printf("Failure changing SDA hold time (%d)\n", ret);
	}
...
static cmd_tbl_t cmd_i2c_sub[] = {
	...
	U_BOOT_CMD_MKENT(write, 7, 0, do_i2c_write, "", ""),
#ifdef CONFIG_DM_I2C
	...
	U_BOOT_CMD_MKENT(sda-hold-time, 1, 1, do_i2c_sda_hold_time, "", ""),
};
...
static char i2c_help_text[] =
	...
	"i2c write memaddress chip address[.0, .1, .2] length [-s] [-p <pg size>] - write memory\n"
	"          to I2C; the -s option selects bulk write in a single transaction\n"
	"          the -p option selects page size\n"
#ifdef CONFIG_DM_I2C
	...
	"i2c sda-hold-time [hold time ns] - show or set I2C SDA hold time";
#endif
