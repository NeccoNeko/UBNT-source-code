...
static int do_spi_flash_wrsr_read(int argc, char * const argv[])
{
	int ret;
    u8 rs;

    ret = spi_flash_cmd_read_status(flash, &rs);
    printf("SF: read wrsr value 0x%x\n", rs);
	return ret == 0 ? 0 : 1;
}

static int do_spi_flash_wrsr_write(int argc, char * const argv[])
{
	int ret;
    u8 ws;
	unsigned long data;
	...
	data = simple_strtoul(argv[1], &endp, 16);
	if (*argv[1] == 0 || *endp != 0)
		return -1;

    ws = (u8)data;
    printf("SF: write wrsr with ws 0x%x\n", ws);

    ret = spi_flash_cmd_write_status(flash, ws);
...
#endif
	else if (!strcmp(cmd, "rwrsr"))
		ret = do_spi_flash_wrsr_read(argc, argv);
	else if (!strcmp(cmd, "wwrsr"))
		ret = do_spi_flash_wrsr_write(argc, argv);
...
U_BOOT_CMD(
	...
	"					  or to start of mtd `partition'\n"
    "sf rwrsr - read WRSR\n"
    "sf wwrsr value - write WRSR with value (ex, 0x34)\n"
	SF_TEST_HELP
);
