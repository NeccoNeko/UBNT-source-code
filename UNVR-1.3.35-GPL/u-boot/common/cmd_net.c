...
static int do_dhcp_release(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	return netboot_common(DHCPRELEASE, cmdtp, argc, argv);
}

U_BOOT_CMD(
	dhcprelease,	1,	1,	do_dhcp_release,
	"release IP address previously leased by DHCP server",
	""
);
#endif
