...
#define put_uci(e, str)						\
	do {							\
		size_t uci_strlen = strlen(str);		\
		*e++ = 77;	/* Vendor Class Identifier */	\
		*e++ = uci_strlen;				\
		memcpy(e, str, uci_strlen);			\
		e += uci_strlen;				\
	} while (0)
			...
			struct in_addr requested_ip)
{
	...
	u16 clientarch;
#endif
#if defined(CONFIG_BOOTP_UCI_ENV)
	char *uci;
#endif
...
static int dhcp_prepare_packet(bool release, struct bootp_hdr *bp_offer)
{
	uchar *pkt, *iphdr;
	struct bootp_hdr *bp;
	...
	if (release) {
		...
		net_copy_ip(&bp->bp_ciaddr, &net_ip);
		extlen = dhcp_extended((u8 *)bp->bp_vend, DHCP_RELEASE,
			dhcp_server_ip, zero_ip);
	} else {
		/*
	...
	if (release) {
		net_set_udp_header(iphdr, dhcp_server_ip, PORT_BOOTPS, PORT_BOOTPC, iplen);
	} else {
		bcast_ip.s_addr = 0xFFFFFFFFL;
		net_set_udp_header(iphdr, bcast_ip, PORT_BOOTPS, PORT_BOOTPC, iplen);
	}
...
static void dhcp_send_request_packet(struct bootp_hdr *bp_offer)
{
	...
	pktlen = dhcp_prepare_packet(false, bp_offer);
...
static void dhcp_send_release_packet(void)
{
	int pktlen;

	debug("dhcp_send_release_packet: Sending DHCPRELEASE\n");
	pktlen = dhcp_prepare_packet(true, NULL);

	debug("Transmitting DHCPRELEASE packet: len = %d\n", pktlen);
	net_send_packet(net_tx_packet, pktlen);
}
...
{
	if ((dhcp_state == BOUND) && dhcp_server_ip.s_addr && net_ip.s_addr) {
		printf("DHCP sending release packet for address %pI4\n", &net_ip);
		dhcp_send_release_packet();
		dhcp_state = INIT;
	}
