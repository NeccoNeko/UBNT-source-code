...
struct i2c_adapter {
	...
	uint		(*set_bus_speed)(struct i2c_adapter *adap,
				uint speed);
	uint		(*set_sda_hold_time)(struct i2c_adapter *adap,
				uint hold_time);
	int		speed;
	int		sda_hold_time;
	int		waitdelay;
...
#define U_BOOT_I2C_MKENT_COMPLETE_V2(_init, _probe, _read, _write, \
		_set_speed, _set_sda_hold_time, _speed, _slaveaddr, _hwadapnr, _name) \
	{ \
		.init			=	_init, \
		.probe			=	_probe, \
		.read			=	_read, \
		.write			=	_write, \
		.set_bus_speed		=	_set_speed, \
		.set_sda_hold_time	=	_set_sda_hold_time, \
		.speed			=	_speed, \
		.sda_hold_time		=	0, \
		.slaveaddr		=	_slaveaddr, \
		.init_done		=	0, \
...
#define U_BOOT_I2C_ADAP_COMPLETE_V2(_name, _init, _probe, _read, _write, \
			_set_speed, _set_sda_hold_time, _speed, _slaveaddr, _hwadapnr) \
	ll_entry_declare(struct i2c_adapter, _name, i2c) = \
	U_BOOT_I2C_MKENT_COMPLETE_V2(_init, _probe, _read, _write, \
		 _set_speed, _set_sda_hold_time, _speed, _slaveaddr, _hwadapnr, _name);
...
/*
 * i2c_set_sda_hold_time:
 *
 *  Change the speed of the active I2C bus
 *
 *	sda_hold_time - SDA hold time in ns
 *
 *	Returns: new bus SDA hold time
 *
 */
unsigned int i2c_set_sda_hold_time(unsigned int sda_hold_time);

/*
 * i2c_get_sda_hold_time:
 *
 *  Returns SDA hold time of currently active I2C bus in ns
 */

unsigned int i2c_get_sda_hold_time(void);
