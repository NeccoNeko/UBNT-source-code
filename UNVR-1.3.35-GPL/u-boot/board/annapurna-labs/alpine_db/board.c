...
static int dt_based_init_gpio_expander(void)
{
	int off;
	const u32 *cell;
	const struct fdt_property *fdt_prop;
...
static int dt_based_init_eth_params_port(int port_idx)
{
	...
	if (!strcmp(prop, "rgmii")) {
		...
		/* LED 10g */
		sprintf(path, "/soc/leds/eth_%u_10g", port_idx);
		off = fdt_path_offset(working_fdt, path);
		fdt_prop = fdt_get_property(working_fdt, off, "gpios", &len);
		if (fdt_prop) {
			int gpio;
			...
			if (gpio == 0)
				printf("GPIO 0 can't be used as 1g/10g speed indication\n");

			debug("\t1G/10G speed indication GPIO: %d\n", gpio);
			params->gpio_spd_1g_10g = gpio;
		} else {
			debug("\tNo gpio for 1g/10g speed indication\n");
		}

		/* LED 25g */
		sprintf(path, "/soc/leds/eth_%u_25g", port_idx);
		off = fdt_path_offset(working_fdt, path);
		fdt_prop = fdt_get_property(working_fdt, off, "gpios", &len);
		if (fdt_prop) {
			int gpio;
			...
			gpio = fdt32_to_cpu(cell[1]);
			if (gpio == 0)
				printf("GPIO 0 can't be used as 25g speed indication\n");

			debug("\t25G speed indication GPIO: %d\n", gpio);
			params->gpio_spd_25g = gpio;
		} else {
...
static void dt_based_init(void)
{
	...
	if (dt_based_init_gpio_expander())
