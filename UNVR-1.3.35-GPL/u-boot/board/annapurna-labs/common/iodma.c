...
static int iodma_pci_init(void **udma_base, void **app_base)
{
	...
	if (al_globals.dev_id == PBS_UNIT_CHIP_ID_DEV_ID_ALPINE_V1)
		devno = pci_find_device(PCI_VENDOR_ID_ANNAPURNALABS, PCI_DEVICE_ID_AL_RAID, 0);
	else
		devno = pci_find_device(PCI_VENDOR_ID_ANNAPURNALABS, PCI_DEVICE_ID_AL_SSMAE, 1);
	if (devno < 0) {
		printf(
...
int iodma_init(void)
{
	struct al_ssm_dma_params raid_udma0_params = { 0 };
	struct al_udma_q_params raid_tx_params = { 0 };
	struct al_udma_q_params raid_rx_params = { 0 };
	void *udma_base = NULL;
	void *app_base = NULL;
	uint32_t err;
	...
	if (al_ssm_dma_init(&raid, &raid_udma0_params)) {
		printf("al_ssm_dma_init failed!\n");
		return -1;
	}
	if (al_ssm_dma_q_init(&raid, DMA_Q_ID, &raid_tx_params, &raid_rx_params, AL_RAID_Q)) {
		printf("al_ssm_dma_q_init failed!\n");
		return -1;
	}
	if (al_ssm_dma_state_set(&raid, UDMA_NORMAL)) {
		printf("al_ssm_dma_state_set failed!\n");
		return -1;
	}
	...
	uint64_t	size)
{
	...
	struct al_buf raid_src_buf = { 0 };
	struct al_block raid_src_block = { 0 };
	struct al_buf raid_dst_buf = { 0 };
	struct al_block raid_dst_block = { 0 };
	struct al_raid_transaction transaction = { 0 };
