	...
	enum al_serdes_reg_page	*page)
{
	...
	else if (!strcmp(page_str, "ptop"))
		*page = AL_SRDS_REG_PAGE_TOP;
	else {
	...
	char *const		argv[])
{
	...
	err = al_globals.serdes[grp].reg_read(
		&al_globals.serdes[grp],
		page,
		type,
		offset,
		&data);
	...
	char *const		argv[])
{
	...
	err = al_globals.serdes[grp].reg_write(
		&al_globals.serdes[grp],
		page,
		type,
		offset,
	...
	char *const		argv[])
{
	...
	al_globals.serdes[grp].pma_hard_reset_group(
		&al_globals.serdes[grp],
		en);
	...
	char *const		argv[])
{
	...
	al_globals.serdes[grp].pma_hard_reset_lane(
		&al_globals.serdes[grp],
		lane,
		en);
	...
	char *const		argv[])
{
	...
	al_globals.serdes[grp].bist_overrides_enable(&al_globals.serdes[grp], rate);
	...
	char *const		argv[])
{
	...
	al_globals.serdes[grp].lane_pm_set(&al_globals.serdes[grp], lane, pm, pm);
	...
	char *const		argv[])
{
	...
	al_globals.serdes[grp].group_pm_set(&al_globals.serdes[grp], pm);
	...
	char *const		argv[])
{
	...
	al_globals.serdes[grp].loopback_control(
		&al_globals.serdes[grp],
		lane,
		lb_mode);
	...
	char *const		argv[])
{
	...
	al_globals.serdes[grp].bist_pattern_select(
		&al_globals.serdes[grp],
		bist_pattern,
		NULL);
	...
	char *const		argv[])
{
	...
	al_globals.serdes[grp].bist_tx_enable(
		&al_globals.serdes[grp],
		lane,
		en);
	...
	char *const		argv[])
{
	...
	al_globals.serdes[grp].bist_tx_err_inject(
		&al_globals.serdes[grp]);
	...
	char *const		argv[])
{
	...
	al_globals.serdes[grp].bist_rx_enable(
		&al_globals.serdes[grp],
		lane,
		en);
	...
	char *const		argv[])
{
	...
	al_globals.serdes[grp].bist_rx_status(
		&al_globals.serdes[grp],
		lane,
		&is_locked,
		&err_cnt_overflow,
		...
		char *const		argv[])
{
	...
	al_globals.serdes[grp].tx_advanced_params_set(
		...
		char *const		argv[])
{
	...
	al_globals.serdes[grp].tx_advanced_params_get(
			&al_globals.serdes[grp], lane, &tx_params);
		...
		char *const		argv[])
{
	...
	for(i = 0; i < 10; ++i) {
		...
		int test_width  = -1;
		int test_height = -1;
		test_score = al_globals.serdes[grp].rx_equalization(
				&al_globals.serdes[grp], lane);
		if(test_score >= best_score)
			al_globals.serdes[grp].calc_eye_size(
					&al_globals.serdes[grp], lane,
		...
		if ((test_score > best_score) ||
			...
			al_globals.serdes[grp].rx_advanced_params_get(
	...
	if(best_score == -1) {
		...
		al_globals.serdes[grp].rx_advanced_params_set(
				&al_globals.serdes[grp], lane, &rx_params);
	...
	char *const		argv[])
{
	...
	for (y = 0; y < 63; y++) {
		...
		for (x = 0 ; x < 64; x++) {
			...
			err = al_globals.serdes[grp].eye_diag_sample(
