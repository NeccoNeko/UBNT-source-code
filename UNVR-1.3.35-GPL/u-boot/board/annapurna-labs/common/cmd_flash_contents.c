...
static int do_flash_contents_obj_validate(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (AL_FLASH_OBJ_ID_ID(obj_id) == AL_FLASH_OBJ_ID_STG2) {
		if (al_globals.dev_id == PBS_UNIT_CHIP_ID_DEV_ID_ALPINE_V1)
			err = al_flash_stage2_validate(dev_read, toc_entry.offset,
					(void *)(uintptr_t)load_addr, size, &obj_hdr);
		else
			err = al_flash_stage2_v2_validate(dev_read, toc_entry.offset,
					(void *)(uintptr_t)load_addr, size, &obj_hdr);
	...
	} else if (AL_FLASH_OBJ_ID_ID(obj_id) == AL_FLASH_OBJ_ID_PRE_BOOT_V2) {
		err = al_flash_pre_boot_v2_validate(dev_read, toc_entry.offset, (void *)(uintptr_t)load_addr, size, &obj_hdr);
		if (err) {
			printf("al_flash_pre_boot_validate failed!\n");
			return 1;
		}
...
static int flash_contents_obj_update(unsigned int obj_id, void *obj_buff, unsigned int size)
{
	...
	if (last_block_num_bytes && ((last_block_addr > first_block_addr) || (!first_block_num_bytes))) {
		...
		if ((dev_write != flash_contents_nand_write) &&
			...
			(AL_FLASH_OBJ_ID_ID(obj_id) == AL_FLASH_OBJ_ID_PRE_BOOT_V2))) {
...
static int do_flash_contents_instance_invalidate(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	err = al_flash_toc_find_id(dev_read, toc_offset, obj_id, 0,
		&found_index, &toc_entry);
	if (err) {
		debug("al_flash_toc_find_id failed!\n");
		goto try_preboot;
	}
	if (found_index < 0) {
		debug("Object not found in TOC!\n");
		goto try_preboot;
	}
...
static struct boot_mode_obj {
	struct al_flash_obj_hdr		header;
	struct al_flash_boot_mode	boot_mode;
	uint32_t			data_checksum;
} boot_mode_obj;
static struct al_flash_boot_mode boot_mode;
static unsigned int boot_mode_obj_id;

static int boot_mode_load(void)
{
	int err;
	...
	boot_mode_obj_id = AL_FLASH_OBJ_ID(AL_FLASH_OBJ_ID_BOOT_MODE, 0);
	err = flash_contents_obj_read(boot_mode_obj_id, &boot_mode_obj.header,
		&boot_mode_obj.boot_mode, sizeof(struct al_flash_boot_mode));
	if (err) {
		printf("%s failed!\n", __func__);
		return 1;
	}

	boot_mode = boot_mode_obj.boot_mode;

	return 0;
}

static int boot_mode_save(void)
{
	int err;
	int i;
	...
	boot_mode_obj.boot_mode = boot_mode;
	...
	err = flash_contents_obj_update(boot_mode_obj_id, &boot_mode_obj,
		sizeof(struct boot_mode_obj));
	if (err) {
		printf("%s failed!\n", __func__);
		return 1;
	}
	...
	cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	boot_mode.default_app_id = obj_id_from_args(argv[1], "0");
	...
	cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	printf("Default app: 0x%x (%s)\n", boot_mode.default_app_id,
		al_flash_obj_id_to_str(boot_mode.default_app_id));
