 ...
 *		mdc_mdio_write (preamableLength, phyID, regID, data)
 *
 * #define MDC_MDIO_READ(preamableLength, phyID, regID, pData) 	\
 *	mdc_mdio_read(preamableLength, phyID, regID, pData)
*/

static void inline mdc_mdio_write(int preamableLength, int phyID, int regID, int data)
{
	int ret;
	struct mii_dev *bus;
	...
	ret = bus->write(bus, phyID, MDIO_DEVAD_NONE, regID, data);
	if (ret < 0)
		printf("failed to write rtl83xx register\n");
}

static void inline mdc_mdio_read(int preamableLength, int phyID, int regID, int *pData)
{
	int ret;
	struct mii_dev *bus;
	...
	ret = bus->read(bus, phyID, MDIO_DEVAD_NONE, regID);
	if (ret >= 0) {
		*pData = ret;
	}
	else  {
		printf("failed to read rtl83xx register\n");
	}
	return;
}
