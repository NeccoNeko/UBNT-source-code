...
/*
 * i2c_dw_set_sda_hold_time - Set the i2c SDA hold time
 * @hold_time_ns:	required hold time [ns]
 *
 * Set the i2c SDA hold time
 */
unsigned int dw_i2c_set_sda_hold_time(struct i2c_adapter *adap, unsigned int hold_time)
{
	struct i2c_regs *i2c_base = i2c_get_base(adap);
	unsigned int reg_val = (hold_time * IC_CLK) / NANO_TO_MICRO;
	unsigned int enbl;

	if (hold_time && (!reg_val)) {
		printf("Invalid hold time!\n");
		return 1;
	}
	...
	writel(reg_val, &i2c_base->ic_sda_hold);

	/* Enable back i2c now SDA hold time is set */
	enbl |= IC_ENABLE_0B;
	writel(enbl, &i2c_base->ic_enable);
...
U_BOOT_I2C_ADAP_COMPLETE_V2(
	dw_0, dw_i2c_init, dw_i2c_probe, dw_i2c_read,
	dw_i2c_write, dw_i2c_set_bus_speed, dw_i2c_set_sda_hold_time,
	CONFIG_SYS_I2C_SPEED, CONFIG_SYS_I2C_SLAVE, 0)

#if CONFIG_SYS_I2C_BUS_MAX >= 2
U_BOOT_I2C_ADAP_COMPLETE_V2(
	dw_1, dw_i2c_init, dw_i2c_probe, dw_i2c_read,
	dw_i2c_write, dw_i2c_set_bus_speed, dw_i2c_set_sda_hold_time,
...
#if CONFIG_SYS_I2C_BUS_MAX >= 3
U_BOOT_I2C_ADAP_COMPLETE_V2(
	dw_2, dw_i2c_init, dw_i2c_probe, dw_i2c_read,
	dw_i2c_write, dw_i2c_set_bus_speed, dw_i2c_set_sda_hold_time,
...
#if CONFIG_SYS_I2C_BUS_MAX >= 4
U_BOOT_I2C_ADAP_COMPLETE_V2(
	dw_3, dw_i2c_init, dw_i2c_probe, dw_i2c_read,
	dw_i2c_write, dw_i2c_set_bus_speed, dw_i2c_set_sda_hold_time,
