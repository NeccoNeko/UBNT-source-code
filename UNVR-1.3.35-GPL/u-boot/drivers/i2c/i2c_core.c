...
void i2c_reloc_fixup(void)
{
	...
	for (i = 0; i < max; i++) {
		...
		addr = (unsigned long)i2c_adap_p->set_sda_hold_time;
		addr += gd->reloc_off;
		i2c_adap_p->set_sda_hold_time = (void *)addr;
		/* name */
...
unsigned int i2c_set_sda_hold_time(unsigned int sda_hold_time)
{
	...
	if (I2C_ADAP->set_sda_hold_time == NULL)
		return 0;
	ret = I2C_ADAP->set_sda_hold_time(I2C_ADAP, sda_hold_time);
	if (gd->flags & GD_FLG_RELOC)
		I2C_ADAP->sda_hold_time = (ret == 0) ? sda_hold_time : 0;
